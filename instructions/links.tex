...
produces a unique message even if the messages being sent are equal. This
is useful in sending control signals, such as those of the failure detector P.

Modify the algorithm so that pl only delivers its own messages, maintaining
the no-creation property. It should ignore sl deliver events for messages not
sent by pl (where pl may be itself, or the pl component on another node). You
may find \verb!make_ref()!\footnote{\url{http://erlang.org/doc/man/erlang.html\#make_ref-0}} useful for generating pseudo-random ids.
